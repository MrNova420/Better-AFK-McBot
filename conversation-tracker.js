class ConversationTracker {
   constructor(config = {}) {
      ...
      this.topics = new Map();
      this.recentContext = [];
      this.maxContextLength = config.contextLength || 10;
      this.maxConversationAge = 30 * 60 * 1000;
   }

   addMessage(playerName, message, isBot = false) {
   ...
   extractTopics(message, playerName) {
      ...
      const topicKeywords = {
         mining: ['mine', 'diamond', 'ore', 'cave', 'strip'],
         building: ['build', 'house', 'base', 'construct', 'design'],
         farming: ['farm', 'crop', 'wheat', 'food', 'animal'],
   ...
   isPlayerInterestedIn(playerName, topic) {
      const conv = this.conversations.get(playerName);
      return conv ? conv.topics.has(topic) : false;
   }
